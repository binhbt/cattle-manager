 ...
 */
package com.leo.cattle.data.cache;

import android.content.Context;
import com.leo.cattle.data.cache.serializer.JsonSerializer;
import com.leo.cattle.data.entity.UserEntity;
import com.leo.cattle.data.exception.UserNotFoundException;
...
public class UserCacheImpl implements UserCache {
  ...
  private static final String SETTINGS_FILE_NAME = "com.leo.cattles.SETTINGS";
  ...
  @Override public Observable<UserEntity> getLoggedUser(){
    return Observable.create(subscriber -> {
      File userEntityFile = UserCacheImpl.this.buildFile(0);
      String fileContent = UserCacheImpl.this.fileManager.readFileContent(userEntityFile);
      UserEntity userEntity = UserCacheImpl.this.serializer.deserialize(fileContent);
  ...
  @Override
  public void putSession(UserEntity userEntity){
    if (userEntity != null) {
      File userEntitiyFile = this.buildFile(0);
      String jsonString = this.serializer.serialize(userEntity);
      this.executeAsynchronously(new CacheWriter(this.fileManager, userEntitiyFile,
              jsonString));
      setLastCacheUpdateTimeMillis();
